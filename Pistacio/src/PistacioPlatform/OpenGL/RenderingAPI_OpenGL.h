...
namespace Pistacio
{
  ...
  public:
    ...
    RendererID CreatePersistenBuffer(const PersistentBufferDescriptor descriptor) const override;
    RendererID CreatePersistenBuffer(const PersistentBufferDescriptor descriptor, const void* data) const override;
    void* MapPersistentBuffer(RendererID rendererId, uint32_t offset, uint32_t size, uint32_t accessFlags) const override;
    ...
    RendererID CreateInputLayout(RendererID indexBuffer, const std::vector<std::tuple<Buffer*, BufferMemoryLayout>>& vertexBuffers, const std::vector<std::tuple<PersistentBuffer*, BufferMemoryLayout>>& vertexPersistentBuffers) const override;
    RendererID CreateInputLayout(const std::initializer_list<std::tuple<Buffer*, BufferMemoryLayout>>& vertexBuffers) const override;
    RendererID CreateInputLayout(RendererID indexBuffer, const std::initializer_list<std::tuple<Buffer*, BufferMemoryLayout>>& vertexBuffers) const override;
    RendererID CreateInputLayout(const std::vector<std::tuple<Buffer*, BufferMemoryLayout>>& vertexBuffers) const override;
    RendererID CreateInputLayout(RendererID indexBuffer, const std::vector<std::tuple<Buffer*, BufferMemoryLayout>>& vertexBuffers) const override;
    void DeleteInputLayout(RendererID& id) const override;

    RendererID CreateSampler(const TextureDescriptorBase& baseTextureDescriptor, const SamplerDescriptor& samplerDescriptor) const override;
    RendererID CreateSampler(const std::string& path, const SamplerDescriptor& samplerDescriptor, TextureDescriptorBase& textureDescriptor_out) const override;
    ...
    void CreateAttachment(const RendererID& framebufferId, const RendererID& attachmentId, const AttachmentPoint& attachmentPoint) const override;
