...
namespace Pistacio
{
  ...
  public:
    ...
    virtual RendererID CreatePersistenBuffer(const PersistentBufferDescriptor descriptor) const = 0;
    virtual RendererID CreatePersistenBuffer(const PersistentBufferDescriptor descriptor, const void* data) const = 0;
    virtual void* MapPersistentBuffer(RendererID rendererId, uint32_t offset, uint32_t size, uint32_t accessFlags) const = 0;
    ...
    virtual RendererID CreateInputLayout(const std::initializer_list<std::tuple<Buffer*, BufferMemoryLayout>>& vertexBuffers) const = 0;
    virtual RendererID CreateInputLayout(RendererID indexBuffer, const std::initializer_list<std::tuple<Buffer*, BufferMemoryLayout>>& vertexBuffers) const = 0;
    virtual RendererID CreateInputLayout(const std::vector<std::tuple<Buffer*, BufferMemoryLayout>>& vertexBuffers) const = 0;
    virtual RendererID CreateInputLayout(RendererID indexBuffer, const std::vector<std::tuple<Buffer*, BufferMemoryLayout>>& vertexBuffers) const = 0;
    virtual RendererID CreateInputLayout(RendererID indexBuffer, const std::vector<std::tuple<Buffer*, BufferMemoryLayout>>& vertexBuffers, const std::vector<std::tuple<PersistentBuffer*, BufferMemoryLayout>>& vertexPersistentBuffers) const = 0;
    virtual void DeleteInputLayout(RendererID& id) const = 0;

    virtual RendererID CreateSampler(const TextureDescriptorBase& baseTextureDescriptor, const SamplerDescriptor& samplerDescriptor) const = 0;
    virtual RendererID CreateSampler(const std::string& path, const SamplerDescriptor& samplerDescriptor, TextureDescriptorBase& textureDescriptor_out) const = 0;
    ...
    virtual void CreateAttachment(const RendererID& framebufferId, const RendererID& attachmentId, const AttachmentPoint& attachmentPoint) const = 0;
