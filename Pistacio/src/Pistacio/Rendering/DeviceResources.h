...
namespace Pistacio
{
  ...
  {
    uint32_t Size = 0;
    BufferUsageFlag Usage = STATIC_DRAW;
  ...
  } PersistentBufferFlagBits;
  using PersistentBufferFlag = uint32_t;

  struct PersistentBufferDescriptor
  {
    uint32_t Size = 0;
    PersistentBufferFlag Flags = 0;
    uint32_t AttributeDivisor = 0;
  };
  ...
  {
    //std::string SemanticName;
    AttachmentPoint AttachmentPoint = COLOR_ATTACHMENT_0;
  };

  struct Attachment
  {
    RendererID RendererID;
    TextureDescriptorBase TextureBase;
